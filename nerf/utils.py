...
class Trainer(object):
    ...
    def prepare_embeddings(self):
        ...
        if self.opt.text is not None:
            ...
            if "IF" in self.guidance:
                self.embeddings["IF"]["default"] = self.guidance["IF"].get_text_embeds(
                    [self.opt.text]
                )
                self.embeddings["IF"]["uncond"] = self.guidance["IF"].get_text_embeds(
                    [self.opt.negative]
                )

                for d in ["front", "side", "back"]:
                    self.embeddings["IF"][d] = self.guidance["IF"].get_text_embeds(
                        [f"{self.opt.text}, {d} view"]
                    )

            if "clip" in self.guidance:
                self.embeddings["clip"]["text"] = self.guidance["clip"].get_text_embeds(
                    self.opt.text
                )
        ...
        if self.opt.images is not None:
            ...
            for image in self.opt.images:
                assert image.endswith(
                    "_rgba.png"
                )  # the rest of this code assumes that the _rgba image has been passed.
            rgbas = [
                cv2.cvtColor(
                    cv2.imread(image, cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGBA
                )
                for image in self.opt.images
            ]
            rgba_hw = np.stack(
                [
                    cv2.resize(rgba, (w, h), interpolation=cv2.INTER_AREA).astype(
                        np.float32
                    )
                    / 255
                    for rgba in rgbas
            ...
            print(
                f"[INFO] dataset: load image prompt {self.opt.images} {self.rgb.shape}"
            )

            # load depth
            depth_paths = [
                image.replace("_rgba.png", "_depth.png") for image in self.opt.images
            ]
            depths = [
            ...
            depth = np.stack(
                [
                    cv2.resize(depth, (w, h), interpolation=cv2.INTER_AREA)
                    for depth in depths
                ]
            )
            self.depth = torch.from_numpy(depth.astype(np.float32) / 255).to(
                self.device
            )  # TODO: this should be mapped to FP16
            print(f"[INFO] dataset: load depth prompt {depth_paths} {self.depth.shape}")

            # load normal   # TODO: don't load if normal loss is 0
            normal_paths = [
                image.replace("_rgba.png", "_normal.png") for image in self.opt.images
            ]
            normals = [
                cv2.imread(normal_path, cv2.IMREAD_UNCHANGED)
                for normal_path in normal_paths
            ]
            normal = np.stack(
                [
                    cv2.resize(normal, (w, h), interpolation=cv2.INTER_AREA)
                    for normal in normals
                ]
            )
            self.normal = torch.from_numpy(normal.astype(np.float32) / 255).to(
                self.device
            )
            print(
                f"[INFO] dataset: load normal prompt {normal_paths} {self.normal.shape}"
            )
            ...
            if "zero123" in self.guidance:
                rgba_256 = np.stack(
                    [
                        cv2.resize(
                            rgba, (256, 256), interpolation=cv2.INTER_AREA
                        ).astype(np.float32)
                        / 255
                        for rgba in rgbas
                    ]
                )
                rgbs_256 = rgba_256[..., :3] * rgba_256[..., 3:] + (
                    1 - rgba_256[..., 3:]
                )
                rgb_256 = (
                    torch.from_numpy(rgbs_256)
                    .permute(0, 3, 1, 2)
                    .contiguous()
                    .to(self.device)
                )
            ...
            if "clip" in self.guidance:
                self.embeddings["clip"]["image"] = self.guidance["clip"].get_img_embeds(
                    self.rgb
                )
    ...
    def train_step(self, data, save_guidance_path: Path = None):
        ...
        if do_rgbd_loss:
            ...
            # color loss
            gt_rgb = gt_rgb * gt_mask[:, None].float() + bg_color.reshape(
                B, H, W, 3
            ).permute(0, 3, 1, 2).contiguous() * (1 - gt_mask[:, None].float())
            loss = self.opt.lambda_rgb * F.mse_loss(pred_rgb, gt_rgb)

            # mask loss
            loss = loss + self.opt.lambda_mask * F.mse_loss(
                pred_mask[:, 0], gt_mask.float()
            )
            ...
            if self.opt.lambda_depth > 0:
                ...
                loss = loss + lambda_depth * (
                    1 - self.pearson(valid_pred_depth, valid_gt_depth)
                )
        ...
        # novel view loss
            ...
            if "SD" in self.guidance:
                ...
                if self.opt.perpneg:
                    ...
                    text_z_comp, weights = adjust_text_embeddings(
                        self.embeddings["SD"], azimuth, self.opt
                    )
            ...
            if "IF" in self.guidance:
                ...
                if self.opt.perpneg:
                    text_z_comp, weights = adjust_text_embeddings(
                        self.embeddings["IF"], azimuth, self.opt
                    )
                    text_z.append(text_z_comp)
                else:
                    for b in range(azimuth.shape[0]):
            ...
            if "zero123" in self.guidance:
                ...
                loss = loss + self.guidance["zero123"].train_step(
                    self.embeddings["zero123"]["default"],
                    pred_rgb,
                    polar,
                    azimuth,
            ...
            if "clip" in self.guidance:
                ...
                lambda_guidance = (
                    10 * (1 - abs(azimuth) / 180) * self.opt.lambda_guidance
                )

                loss = loss + self.guidance["clip"].train_step(
                    self.embeddings["clip"], pred_rgb, grad_scale=lambda_guidance
                )
        ...
        if not self.opt.dmtet:
            ...
            if (
                self.opt.lambda_3d_normal_smooth > 0
                and "loss_normal_perturb" in outputs
            ):
        ...
        return pred_rgb, pred_depth, loss, as_latent
    ...
    def test_gui(
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=self.fp16):
                ...
                preds, preds_depth, _ = self.test_step(
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        for data in loader:
            ...
            # update grid every 16 steps
            if (
                self.model.cuda_ray or self.model.taichi_ray
            ) and self.global_step % self.opt.update_extra_interval == 0:
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    self.model.update_extra_state()
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                ...
                pred_rgbs, pred_depths, loss, as_latent = self.train_step(
