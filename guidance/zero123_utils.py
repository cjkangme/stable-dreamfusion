...
class Zero123(nn.Module):
    def __init__(
        ...
        self.model = load_model_from_config(
            self.config, ckpt, device=self.device, vram_O=vram_O
        )
    ...
    def get_img_embeds(self, x):
        ...
        x = x * 2 - 1
        c = [
            self.model.get_learned_conditioning(xx.unsqueeze(0)) for xx in x
        ]  # .tile(n_samples, 1, 1)
    ...
    def train_step(
        ...
        v1 = torch.stack(
            [
                radius + ref_radii[0],
                torch.deg2rad(polar + ref_polars[0]),
                torch.deg2rad(azimuth + ref_azimuths[0]),
            ],
            dim=-1,
        )  # polar,azimuth,radius are all actually delta wrt default
        v2 = torch.stack(
            [
                torch.tensor(ref_radii),
                torch.deg2rad(torch.tensor(ref_polars)),
                torch.deg2rad(torch.tensor(ref_azimuths)),
            ],
            dim=-1,
        )
        ...
        if self.opt.zero123_grad_scale == "angle":
            grad_scale = (
                angles.min(dim=1)[0] / (180 / len(ref_azimuths))
            ) * grad_scale  # rethink 180/len(ref_azimuths) # claforte: try inverting grad_scale or just fixing it to 1.0
        elif self.opt.zero123_grad_scale == "None":
            grad_scale = 1.0  # claforte: I think this might converge faster...?
        else:
            assert (
                False
            ), f"Unrecognized `zero123_grad_scale`: {self.opt.zero123_grad_scale}"
        ...
        if len(ref_azimuths) > 1:
            ...
            inv_angles = torch.tensor([1.0]).to(self.device)

        # Multiply closeness-weight by user-given weights
        zero123_ws = (
            torch.tensor(embeddings["zero123_ws"])[None, :].to(self.device) * inv_angles
        )
        ...
        with torch.no_grad():
            ...
            for (
                ...
                T = torch.stack(
                    [
                        torch.deg2rad(p),
                        torch.sin(torch.deg2rad(-a)),
                        torch.cos(torch.deg2rad(a)),
                        r,
                    ],
                    dim=-1,
                )[:, None, :]
                ...
                clip_emb = self.model.cc_projection(
                    torch.cat([c_crossattn.repeat(len(T), 1, 1), T], dim=-1)
                )
                cond["c_crossattn"] = [
                    torch.cat(
        ...
        noise_pred = (
            torch.stack(noise_preds).sum(dim=0)
            / zero123_ws.sum(dim=-1)[:, None, None, None]
        )
    ...
    @torch.no_grad()
    def visualize(self, data, embeddings, epoch, pred_rgbs):
        if pred_rgbs.shape[1] == 4:
            latents = (
                F.interpolate(pred_rgbs, (32, 32), mode="bilinear", align_corners=False)
                * 2
                - 1
            )
        else:
            pred_rgb_256 = F.interpolate(
                pred_rgbs, (256, 256), mode="bilinear", align_corners=False
            )
            latents = self.encode_imgs(pred_rgb_256)
        ...
        latents_denoised = self.model.predict_start_from_noise(
            latents_noisy, t, noise_pred
        )

        images = self.decode_latents(latents)
        noisy_images = self.decode_latents(latents_noisy)
        denoised_images = self.decode_latents(latents_denoised)
        ...
        save_image(viz_images, f"./result/epoch{epoch}.png")
    ...
    def __call__(
        ...
        polar=0,
        azimuth=0,
        radius=0,  # new view params
        scale=3,
        ddim_steps=50,
        ddim_eta=1,
        h=256,
        w=256,  # diffusion params
        c_crossattn=None,
        c_concat=None,
        post_process=True,
    ):
