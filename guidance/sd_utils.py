...
class StableDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def visualize(self, text_embeddings, pred_rgb, epoch, guidance_scale=100):
        if pred_rgb.shape[1] == 4:
            ...
            # encode image into latents with vae, requires grad!
            latents = self.encode_imgs(pred_rgb_512.type(self.precision_t))
        ...
        tt = torch.cat([t] * 2)
        noise_pred = self.unet(
            latent_model_input.type(self.precision_t),
            tt,
        ...
        # TODO: also denoise all-the-way
        pred_rgb_512 = self.decode_latents(latents.type(self.precision_t))
